...
public class WebAuctionBlockListener implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if(getTypeId(block) == 63 || getTypeId(block) == 68) {
			...
			if(thisSign.getLine(0).equals("[WebAuctionPlus]")) {
				if(!p.hasPermission("rm.remove")) {
	...
	public void onSignChange(SignChangeEvent event) {
		...
		if(p == null) return;
		if (!lines[0].equalsIgnoreCase("[WebAuctionPlus]") &&
			!lines[0].equalsIgnoreCase("[WebAuctionPlus]") &&
			!lines[0].equalsIgnoreCase("[WebAuctionPlus]") ) return;
		event.setLine(0, "[RhythmMarket]");

		// Shout sign
		if(lines[1].equalsIgnoreCase("Shout")) {
			if(!p.hasPermission("rm.create.sign.shout")) {
				NoPermission(event);
				return;
			}
		...
		if(lines[1].equalsIgnoreCase("Recent")) {
			if(!p.hasPermission("rm.create.sign.recent")) {
				NoPermission(event);
				return;
			}
...
//			}
		...
		if(lines[1].equalsIgnoreCase("Deposit")) {
			if(!p.hasPermission("rm.create.sign.deposit")) {
				NoPermission(event);
				return;
			}
		...
		if(lines[1].equalsIgnoreCase("Withdraw")) {
			if(!p.hasPermission("rm.create.sign.withdraw")) {
				NoPermission(event);
				return;
			}
		...
		if(lines[1].equalsIgnoreCase("MailBox") ||
			...
			if(!p.hasPermission("rm.create.sign.mailbox")) {
				NoPermission(event);
				return;
			}
